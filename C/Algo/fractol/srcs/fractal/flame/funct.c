...
void			linear(t_env *env, double x, double y) /* Linear */
{
	env->z.r = x;
	env->z.i = y;
}
...
void			sinus(t_env *env, double x, double y)
{		
	env->z.r = sin (x);
	env->z.i = sin (y);
}
...
void			hand(t_env *env, double x, double y){ 6/* Handkerchief */
	r = sqrt (x * x + y * y);
	theta = atan2 (y, x);
	env->z.r = r * sin (theta + r);
	env->z.i = r * cos (theta - r);
}
void			heart(t_env *env, double x, double y){ 7/* Heart */
	r = sqrt (x * x + y * y);
	theta = atan2 (y, x);
	env->z.r = r * sin (theta * r);
	env->z.i = -r * cos (theta * r);
